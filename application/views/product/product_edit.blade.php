        ...
        <div class="data-container">
            <header>
                ...
                <ul class="data-header-actions">
                    <li>
                        <a class="btn btn-inverse" href="{{$_SERVER['HTTP_REFERER']}}">返回</a>
                    </li>
                </ul>
            </header>
            <section>
                <form method="post">
                    <fieldset>
                        <div class="control-group">
                            ...
                            <div class="controls">
                                <input id="product_name" name="product_name" class="input-xlarge" type="text" value="{{Input::old('product_name',$product->product_name)}}">
                        ...
                        <div class="control-group">
                            ...
                            <div class="controls">
                                <label class="checkbox">
                                    <input id="status" name="status" {{Input::old('status',$product->status) != 1 ? '' : 'checked="checked"'}} type="checkbox" value="1">
                                    生效
                                </label>
                            </div>
                        </div>
                        <div class="control-group">
                            ...
                            <div class="controls">
                                <label class="checkbox">
                                    <input id="is_suggest" name="is_suggest" {{Input::old('is_suggest',$product->is_suggest) != 1 ? '' : 'checked="checked"'}} type="checkbox" value="1">
                                    推荐
                                </label>
                            </div>
                        </div>
                        <div class="control-group">
                            ...
                            <div class="controls">
                                <input id="sort" name="sort" class="input-xlarge" type="text" value="{{Input::old('sort',$product->sort)}}">
                        ...
                        <div class="control-group">
                            ...
                            <div class="controls">
                                <select id="product_type" name="product_type">
                                    ...
                                    <option {{$type->id == Input::old('prodcut_type',$product->product_type_id) ? 'selected="selected"' : ''}} value="{{$type->id}}">{{$type->top_name->name.'-'.$type->name}}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        <div class="control-group">
                            ...
                            <div class="controls">
                                <textarea class="input-xlarge" name="info" rows="3" style="width: 500px">{{Input::old('info',$product->info)}}</textarea>
                                ...
                                <table class="table table-bordered table-hover" style="word-break:break-all;">
                                    ...
                                    <tbody id="attribute_list">
                                        ...
                                        <tr id="attribute_list_id_{{$i}}">
                                            <td id="sort">{{$attribute->pivot->sort}}</td>
                                            <td true_id="{{$attribute->id}}" id="attribute_id">{{$attribute->name}}
                                            <input type="hidden" name="product_attribute[]" value="{{$attribute->id}}||{{trim($attribute->pivot->value,'"')}}||{{$attribute->pivot->sort}}||{{$attribute->pivot->display}}"></td>
                                            <td id="value">{{trim($attribute->pivot->value,'"')}}</td>
                                            <td id="display" true_display="{{$attribute->pivot->display}}">{{$attribute->pivot->display == 1 ? '显示' : '折叠'}}</td>
                                            <td class="toolbar">
                                                <div class="btn-group">
                        ...
                        <div class="control-group">
                            ...
                            <div class="controls">
                                <ul id="image_list" class="thumbnails">
                                    ...
                                    <li id="image_id_{{$im}}" class="span2">
                                        <a class="thumbnail" href="#"><img alt="" id="img" src="{{$image->data_url}}"></a>
                                        <input type="hidden" name="image[]" value="{{$image->data_url}}">
                                        <p style="text-align: center">
                                            <a onclick="remove_image({{$im}})" class="label label-important">删除</a>
                                        </p>
                                    </li>
                                    <?php $im++;?>
                                    @endforeach
                            ...
                            <div class="controls">
                                <ul class="thumbnails" id="video_list">
                                    <li id="file_list">
                                        ...
                                        <a id="file_url" target="_blank" href="{{$product->video_url}}">视频地址</a>
                                        <input type="hidden" name="video_url" value="{{$product->data_video_url}}">
                                        @endif
                                    </li>
                                </ul>
                                <a class="btn btn-danger" id="add_file">添加视频</a>
                                <a onclick="del_file()" class="btn btn-inverse" id="del_file" {{$product->video_url != null ? '' : 'style="display: none"'}}>删除</a>
                            </div>
                        </div>
                        <div class="control-group">
                            ...
                            <div class="controls">
                                <?php
                                    $tag_array =array();
                                    foreach($product->product_tag as $tag_now)
                                    {
                                        $tag_array[] = $tag_now->id;
                                    }
                                ?>
                                @foreach($tags as $tag)
                                <input id="tags" type="checkbox" {{in_array($tag->id,$tag_array) ? 'checked="checked"' : ''}} name="product_tags[]" value="{{$tag->id}}">
                                {{$tag->name}}&nbsp;
                                @endforeach
